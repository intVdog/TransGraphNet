...
import random
import numpy as np
import copy  # [Added] Import copy module to save the best model weights
...
def train(model, lr, num_epochs, X_train, Y_train, X_val, Y_val, edge_index, rt_meas_dim=78, device='cuda', patience=6):
    ...
    # Ensure the model is also on the device (double insurance)
    model.to(device)
    ...
    # Whether training ended due to early stopping or completed all epochs, reload the best weights
...
def evaluate_loss_acc(model, X, y, criterion, edge_index, device='cuda'):
    ...
    # [Fix 3] Ensure evaluation data is on device
    X = X.to(device)
    y = y.to(device)
    edge_index = edge_index.to(device)
...
# evaluate_acc and evaluate_performance basically don't need major changes since they call predict_prob
# But for safety, add .to(device) checks - though if predict_prob handles it, it's usually fine here
...
def evaluate_performance(models, X, y, edge_index, device='cuda'):
    ...
    for name, model in models.items():
        ...
        # Ensure y shape matches pred_ts
        # Note: If y is full node labels but pred_ts is masked, need to align here
        # According to previous logic, predict_prob returns masked results (len(mask))
        # So y should also take masked part, or be pre-sliced during dataset generation
        # Assume y is already masked or shape-matched when passed in

        y_target = y  # If y is full graph, may need y[:, mask] here

        accuracy = (pred_ts == y_target).sum().item() / (y_target.shape[0] * y_target.shape[1])

        # Move back to CPU for sklearn calculations (sklearn doesn't support GPU tensors)
