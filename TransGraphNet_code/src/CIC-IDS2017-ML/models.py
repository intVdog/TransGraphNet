...
class DilatedCNNBranch(nn.Module):
    ...
    Parameters:
        input_dim (int): Dimension of input features (e.g., 78 or 80 for CICIDS-2017)
    ...
    def __init__(self, in_dim, hidden_dim, out_dim, dropout_rate=0.3, reshape_dims=(12, 12)):
        ...
        for i, dilation in enumerate(dilation_rates):
            # Dynamically calculate output channels, gradually increase complexity, stabilize at hidden_channels
            ...
            in_channels = out_channels  # Input channels of next layer = output channels of current layer
        ...
        # Final Dropout and fully connected layer (optional, not needed for direct fusion, output pure feature vector here)
    ...
    """
        ...
        # Auto squeeze when out_dim=1 for convenient criterion calculation
        if x.size(-1) == 1:
            x = x.squeeze(-1)  # → (B, 7)
...
class MainModel(nn.Module):
    def __init__(self, in_dim, hidden_dim, out_dim, choice):
        ...
        self.rt_meas_dim = 78
        # 1. Instantiate each branch model (done only once during initialization)
    ...
    def forward(self, x, edge_index):
        ...
        # Transformer usually only needs x (depends on your TransformerModel implementation)
        if (self.choice != 2):
        ...
        # Note: Pass computed feature tensors here, not raw data
