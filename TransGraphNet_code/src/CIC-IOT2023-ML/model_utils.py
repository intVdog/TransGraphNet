...
import random
import numpy as np
import copy  # [New] Import copy module to save best model weights
...
def train(model, lr, num_epochs, X_train, Y_train, X_val, Y_val, edge_index, rt_meas_dim, device='cuda', patience=6):
    ...
    dataloader = DataLoader(dataset, batch_size=240, shuffle=True)
    ...
    # ==================== [New] Early stopping initialization variables ====================
    ...
    for epoch in range(num_epochs):
        ...
        # ==================== [New] Core logic for early stopping ====================
    ...
    # ==================== [New] Load best weights at the end ====================
...
def predict_prob(model, X, edge_index, rt_meas_dim, device='cuda'):
...
# because they call predict_prob. However, for safety, we can also add .to(device) checks,
# but as long as predict_prob handles it, it's usually fine here
def evaluate_acc(model, X, y, edge_index, device='cuda'):
    prob = predict_prob(model, X, edge_index,46 ,device=device)
...
def evaluate_performance(models, X, y, edge_index, device='cuda'):
    ...
    for name, model in models.items():
        ...
        # So y should also take the masked part, or it's already sliced when generating the dataset
        # Assume y is already masked when passed in, or shapes match
