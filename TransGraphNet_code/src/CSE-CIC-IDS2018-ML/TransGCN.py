...
class AdaptiveFusionModule(nn.Module):
    ...
    def __init__(self, hidden_dim, out_dim, num_branches=3):
        ...
        # 1. Scoring network: Score features from each branch
    ...
    def forward(self, features_list):
        ...
        for f in features_list:
            if f.dim() == 1:  # ← Handle cases like (26,)
                f = f.unsqueeze(-1)  # Convert to (26, 1), will be auto-aligned later
        ...
        # To apply Linear, we need to reshape (Batch, Nodes, Dim) to (Batch*Nodes, Dim)
        # Or use torch.nn.functional.linear with broadcasting, but reshaping is most reliable
        ...
        # Stack: [Batch, Num_Branches, Nodes, Dim]  <-- Note the added Nodes dimension
        # Previous code stacked on dim=1 to get [Batch, Branches, Dim], which was for globally pooled features
        # For node-level classification tasks, we need to calculate attention at each node
        ...
        # We need to score each branch at each node
        ...
        # Restore shape: (Batch, Nodes, Branches, 1) -> then transpose for Softmax
        scores = scores.view(b, n, branches, 1)
        # Transpose to (Batch, Nodes, Branches, 1) for Softmax on Branches dimension
        # The above view already gives (B, N, Br, 1), directly apply softmax on dim=2 (Branches)
        ...
        # If input is single sample without batch dimension (original logic processes sample by sample),
        # we can squeeze the batch dimension
        # But for unified training loop, it's usually better to keep (1, Nodes, Out_Dim) for upper layers to squeeze
        return output