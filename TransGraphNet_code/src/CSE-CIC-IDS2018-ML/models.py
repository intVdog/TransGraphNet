...
class TransformerModel(nn.Module):
    ...
    def forward(self, x, mask=None):
        ...
        Returns: (Num_Nodes, Out_Dim) or (Batch, Num_Nodes, Out_Dim)
                 Keep node dimension for subsequent filtering with action_mask
        """
        ...
        # But for unified batch processing in training loop, keeping 3D or letting training loop handle squeeze is better
...
class DilatedCNNBranch(nn.Module):
    ...
    Parameters:
        ...
        hidden_channels (int): Number of hidden layer channels, default 64 as per paper
